...
class GrundyRecGplusGequalsP{
    ...
    /**
     * List of recorded losing states
     */
    ArrayList<ArrayList<Integer>> losingStates = new ArrayList<ArrayList<Integer>>();

    /**
     * List of recorded winning states
     */
	ArrayList<ArrayList<Integer>> winningStates = new ArrayList<ArrayList<Integer>>();
	...
	/**
     * MÃ©thode RECURSIVE qui indique si la configuration (du jeu actuel ou jeu d'essai) est perdante
     * 
     * @param jeu plateau de jeu actuel (l'Ã©tat du jeu Ã  un certain moment au cours de la partie)
     * @return vrai si la configuration (du jeu) est perdante, faux sinon
     */
    boolean estPerdante(ArrayList<Integer> jeu) {
	
        boolean ret = true; // par dÃ©faut la configuration est perdante
        ...
        if (jeu == null) {
			...
			// alors la situation est forcÃ©ment perdante (ret=true) = FIN de la rÃ©cursivitÃ©
            if ( !estPossible(jeu) ) {
			...
			// si le plateau de jeu (passÃ© en paramÃ¨tre) est CONNU comme Ã©tant PERDANT
			// alors on retourne immÃ©diatement "true" et on arrÃªte la rÃ©cursivitÃ© (donc on gagne du temps !)
			else if ( isKnownLosing ( jeu ) ) {

				ret = true;

			} else if ( isKnownWinning ( jeu ) ) {
			...
			// sinon il faut poursuivre la dÃ©composition des tas en crÃ©ant le jeu d'essai suivant
			else {
			
				// crÃ©ation d'un jeu d'essais qui va examiner toutes les dÃ©compositions
				// possibles Ã  partir de jeu
                ArrayList<Integer> essai = new ArrayList<Integer>(); // size = 0
				
				// toute premiÃ¨re dÃ©composition : enlever 1 allumette au premier tas qui possÃ¨de
                ...
                while ( (ligne != -1) && ret) {
                    ...
                    cpt++;
					// mise en oeuvre de la rÃ¨gle numÃ©ro1
					// Une situation (ou position) est dite perdante pour la machine (ou le joueur, peu importe) si et seulement si TOUTES 
					// ses dÃ©compositions possibles (c-Ã -d TOUTES les actions qui consistent Ã  dÃ©composer un tas en 2 tas inÃ©gaux) sont 
					// TOUTES gagnantes pour lâ€™adversaire.
					// Si UNE SEULE dÃ©composition (Ã  partir du jeu) est perdante (pour l'adversaire) alors la configuration n'EST PAS perdante.
					// Ici l'appel Ã  "estPerdante" est RECURSIF.
                    if (estPerdante(essai) == true) {	

                        ret = false;
						
                    } else {
						// gÃ©nÃ¨re la configuration d'essai suivante (c'est-Ã -dire UNE dÃ©composition possible)
						// Ã  partir du jeu, si ligne = -1 il n'y a plus de dÃ©composition possible
                        ligne = suivant(jeu, essai, ligne);
                    }
                }
				
				// si la configuration "essai" est perdante (par exemple [4] ou [3, 3] ou [3, 3, 4]) alors elle est 
				// ajoutÃ©e dans le tableau des positions perdantes
				if (ret) {
	...
	/* ArrayList<Integer> simplify(ArrayList<Integer> state) {

		ArrayList<Integer> copie = normalize(state);

		int[] types  = {
			0,0,0,1,0,2,1,0,2,1,0,2,1,3,2,1,3,2,4,3,0,4,3,0,
			4,3,0,4,1,2,3,1,2,4,1,2,4,1,2,4,1,5,4,1,5,4,1,5,4,
			1,0,2,1,0,2,1,5,2,1,3,2,1,3,2,4,3,2,4,3,2,4,3,2,4,
			3,2,4,3,2,4,5,2,4,5,2,4,3,7,4,3,7,4,3,7,4,3,5,2,3,
			5,2,3,5,2,3
		};
		...
		for (int i = 0; i < ret.size(); i++) {
			...
			for (int j = (i + 1); j < ret.size(); j++) {
				...
				if ((ret.get(j) < types.length) && (ret.get(i) < types.length)) {

					if (types[ret.get(i)] == types[ret.get(j)]) {
    ...
    /**
     * Determines if the configuration is known as a losing configuration in losingStates
     * 
     * @param jeu the current game state
     * @return if the game is in losingStates
     */
    boolean isKnownLosing ( ArrayList<Integer> jeu ) {

        ArrayList<Integer> copie = normalize(jeu);

        boolean ret = false;

        int i = 0;

        while ( !ret && i < losingStates.size() ) {

            if (areSame(copie, losingStates.get(i))) {
    ...
    /**
     * Determines if the configuration is known as a winning configuration in winningStates
     * 
     * @param jeu the current game state
     * @return if the game is in winningStates
     */
    boolean isKnownWinning ( ArrayList<Integer> jeu ) {
        ...
        while ( !ret && i < winningStates.size() ) {

            if (areSame(copie, winningStates.get(i))) {
    ...
    /**
     * Removes all the 1 and 2 from the game and removes all the losing states from the game
     * 
     * @param state the current game state
     * @return the normalized game state
     */
    ArrayList<Integer> normalize ( ArrayList<Integer> state ) {
        ...
        for (int i = 0; i < losingStates.size(); i++) {

            ArrayList<Integer> tasPerdant = losingStates.get(i);

            if (tasPerdant.size() < ret.size() && isSubPart(ret, tasPerdant)) {

                ret.removeAll(tasPerdant);
    ...
    /**
     * Chcks if all the elements of subPart are in tab
     * 
     * @param tab the main ArrayList
     * @param subPart the sub-par we want to check
     * @return true if the subPart is in tab
     */
    boolean isSubPart(ArrayList<Integer> tab, ArrayList<Integer> subPart) {

        boolean ret = true;
        int i = 0;

        while (i < subPart.size() && ret) {

            int frequency = Collections.frequency(subPart, subPart.get(i));

            int ind = tab.indexOf(subPart.get(i));

            if ((ind == -1) || (Collections.frequency(tab, subPart.get(i)) < frequency)) {
    ...
    void testNormalize() {
        ...
        testCaseNormalize(state1, expected1);
        ...
        testCaseNormalize(state2, expected2);

        System.out.println("\nEdge Cases : ");

        ArrayList<Integer> state3 = new ArrayList<Integer>();
        state3.add(1);
        state3.add(2);

        ArrayList<Integer> expected3 = new ArrayList<Integer>();

        testCaseNormalize(state3, expected3);

        ArrayList<Integer> state4 = null;

        ArrayList<Integer> expected4 = new ArrayList<Integer>();

        testCaseNormalize(state4, expected4);

    }

    /**
     * Tests a call to the normalize() method
     * 
     * @param state the current game state
     * @param expected the expected normalized game state
     */
    void testCaseNormalize(ArrayList<Integer> state, ArrayList<Integer> expected) {

        ArrayList<Integer> ret = normalize(state);
        
        System.out.print("normalize(" + state + ") = " + ret);
    ...
    /**
     * Checks if the two SORTED states are the same 
     * 
     * @param state1 the first state
     * @param state2 the second state
     * @return true if the two states are the same
     */
    boolean areSame ( ArrayList<Integer> state1, ArrayList<Integer> state2 ) {

        boolean ret = false;

        if (state1 == null || state2 == null) {

            System.err.println("areSame(): null parameter");
    ...
    void testAreSame() {
        ...
        testCaseAreSame(state1, state2, true);

        ArrayList<Integer> state4 = new ArrayList<Integer>();
        state4.add(1);
        state4.add(2);
        state4.add(4);

        testCaseAreSame(state2, state4, false);

        System.out.println("\nEdge Cases : ");

        ArrayList<Integer> state5 = new ArrayList<Integer>();
        state5.add(1);
        state5.add(2);
        state5.add(3);

        ArrayList<Integer> state6 = new ArrayList<Integer>();
        state6.add(1);
        state6.add(2);

        testCaseAreSame(state5, state6, false);

        ArrayList<Integer> state7 = new ArrayList<Integer>();
        state7.add(1);
        state7.add(2);
        state7.add(3);

        ArrayList<Integer> state8 = new ArrayList<Integer>();
        state8.add(1);
        state8.add(2);
        state8.add(3);
        state8.add(4);

        testCaseAreSame(state7, state8, false);

        ArrayList<Integer> state9 = null;

        ArrayList<Integer> state10 = new ArrayList<Integer>();
        state10.add(1);
        state10.add(2);
        state10.add(3);

        testCaseAreSame(state9, state10, false);

        ArrayList<Integer> state11 = new ArrayList<Integer>();
        state11.add(1);
        state11.add(2);
        state11.add(3);

        ArrayList<Integer> state12 = null;

        testCaseAreSame(state11, state12, false);

    }

    /**
     * Tests a call to the areSame() method
     * 
     * @param state1 the first state
     * @param state2 the second state
     * @param expected the expected result
     */
    void testCaseAreSame(ArrayList<Integer> state1, ArrayList<Integer> state2, boolean expected) {

        boolean ret = areSame(state1, state2);

        System.out.print("areSame(" + state1 + ", " + state2 + ") = " + ret);
